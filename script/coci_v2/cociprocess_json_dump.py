...
class Cocirefprocess:
    ...
    def init_dirs_skeleton(self):
        ...
        self.init_csv("%sd-%s.csv"%(self.OUT_DATA_PATH,str(0)),'oci,citing,cited,creation,timespan,journal_sc,author_sc')
    ...
    def init_issn_dic(self):
        with open(self.INDEX_ISSN_GLOBAL_CSVPATH,'r') as csvfile:
            csv_reader = csv.DictReader(csvfile)
            for row in csv_reader:
                if row['id'] in self.issn_dic:
                    self.issn_dic[row['id']].append(row['value'])
                else:
                ...
                if csvdata != -1:
                    if "errors" in csvdata:
                        ...
                        if (self.datacsv_counter // self.MAX_DATA_ENTRIES == 1):
                            self.datacsv_counter = 0
                            self.file_id += 1
                            self.init_csv("%sd-%s.csv"%(self.OUT_DATA_PATH,str(self.file_id)),'oci,citing,cited,creation,timespan,journal_sc')
    ...
    def process_ref_entry(self,obj):
        ...
        if "DOI" not in obj :
            ...
            #check Journal self Citation
            cited_issn = []
            if cited_doi in self.issn_dic:
                cited_issn = self.issn_dic[cited_doi]

            #check orcids
            cited_orcids = self.build_orcid_list(obj,cited_doi)
