...
class Cociprocess:
    ...
    def find_work(self,query_txt):
        #call cross ref
        res = self.get_data(query_txt, num_iterations=self.NUMBER_ITERATIONS, sleep_time=self.REQ_SLEEP_TIME, req_timeout= self.REQUEST_TIMEOUT)
        ...
        if "errors" not in res:
            try:
                ...
                if "score" in work_item:
                    if work_item["score"] > self.MIN_SCORE:
        ...
        if (obj != -1):
            if "errors" in obj:
                ...
                if "DOI" in obj:
                    ...
                    #first case is true only if self.find_work has been called before
...
if __name__ == "__main__":
    ...
    if args.lookup_file_path:
        cp.LOOKUP_CSV = args.lookup_file_path
    ...
    for subdir, dirs, files in os.walk(cp.INPUT_DATA_PATH):
        for file in files:
